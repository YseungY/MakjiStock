...
function buildMarkdownReport(report) {
  const lines = [
    ...
    `- 환율 조정: clamp(환율하락률 × ${report.pricing.fxWeight} × ${report.pricing.fxScale}, -${report.pricing.fxSurchargeCapPct}, +${report.pricing.fxDiscountCapPct})`,
    `- 총 할인율: min(${report.pricing.discountCapPct}, 검색 쿠폰 + 환율 조정)`,
    `- 가격 범위: 최대 ${report.pricing.discountCapPct}% 할인, 이론상 최대 ${report.pricing.fxSurchargeCapPct}% 할증`,
