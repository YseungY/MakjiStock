...
- 검색 쿠폰은 상품별 Naver 검색지수 절댓값에 `0.1`을 곱해 0~10%p로 계산합니다.
- 환율 조정은 `환율하락률 × 0.28 × 50`이며 ±28%p로 제한합니다.
- 총 할인율은 `min(38, 검색 쿠폰 + 환율 조정)`입니다. 환율 상승 시 할인율이 음수가 되어 할증될 수 있습니다.
- 최종 판매가는 정가 대비 최대 38% 할인이고, 이론상 최대 28% 할증입니다.
